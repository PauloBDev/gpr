...
def calibrate_gauge(filename,folder_path):
    ...
    if(os.path.exists('configs/' + gauge_name + '.yaml')):
        ...
        with open(r'configs/' + gauge_name + '.yaml', 'r') as f:
            ...
            return conf[0]['min_angle'], conf[0]['max_angle'], conf[0]['min_value'], conf[0]['max_value'], conf[0]['units'], x, y, r

    min_angle = input('min_angle: ')
    max_angle = input('max_angle') #highest possible angle
    min_value = input('min_value: ') #usually zero
    max_value = input('max_value: ') #maximum reading of the gauge
    units = input('units: ')
