...
def avg_circles(circles, b):
    ...
    avg_x, avg_y, avg_r = int(avg_x/(b)), int(avg_y/(b)), int(avg_r/(b))
...
def h_transforms(img, min_angle, max_angle, min_value, max_value, units):
	...
	for i in range(0,interval):
		for j in range(0,2):
			if (j % 2 == 0): p1[i][j] = x + 0.9 * r * np.cos(separation * i * 3.14 / 180) #point for lines
			else: p1[i][j] = y + 0.9 * r * np.sin(separation * i * 3.14 / 180)
	...
	thresh, maxValue = 175, 255

	# apply thresholding which helps for finding lines
	th, dst2 = cv2.threshold(gray2, thresh, maxValue, cv2.THRESH_BINARY_INV);

	# find lines
	minLineLength, maxLineGap = 10, 0
